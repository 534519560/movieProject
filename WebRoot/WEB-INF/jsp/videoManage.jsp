...
/*  $(function(){
 ...
 var title=$(".title").text();
   title.replace(new RegExp(/keyword/g),"<em  class='keyword' >"+keyword+"</em>");
   var a=new RegExp(keyword);
   console.debug("<em  class='keyword' >"+keyword+"</em>");
   console.debug(a);
  $(".title").text(title);
   console.debug(title);
 }); */
 
  function videoUpdate(a,b,c){

  var uuid =$("#"+a).val();
  var checkstatus=$("#"+b).val();
    $.post("updatevideobyadmin",{
     "videouuid":uuid,
     "checked":checkstatus
  },function(data,status){
  
     if(data==1){
      $("#"+c).attr("src","source/videopass.png");
  
     }else if(data==2){
      
     $("#"+c).attr("src","source/nopass.png");
     }
  
  });
 
 }
 function toDeleteVideo(a,b){
 console.debug(a);
 var uuid =$("#"+b).val();
    $.post("deletevideobyadmin",{
      "videouuid":uuid,
    },function(data,status){
    console.debug(data);
      if(data=='true'){
  ...
  <a href="uploadVideo" class="btn btn-link" style="position:absolute; right:50px;z-index: 100">上传视频</a>
   <div style="height:44px;" class="js-search-wrap">
        ...
        <div id="header-search">
          <div class="logo-input">
            ...
            <div id="search-block">
              ...
              <form action="queryvideo/default" id="inputform" method="post">
           ...
           <input type="text" value="${serchkeyword}" id="search-keyword" autocomplete="off" name="keyword">
              ...
              <div id="search-button">
                ...
                <span><a href="javascript:inputForm()" style="color: white;">搜索</a></span>
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="so-wrap" style="positon:relative">
     <div class="filter-block">
      ...
      <li class="filter-item ${queryVideoList.checked==-1 ? 'active':'' }" id="li1">
      <a href="javascript:void(0)" onclick="typeserch(null,'li1');">视频管理</a>
      </li>
      <li class="filter-item ${queryVideoList.checked==0 ? 'active':'' }"  id="li2">
      <a href="javascript:void(0)" onclick="typeserch(0,'li2');">待审查视频</a>
      </li>
      <li class="filter-item ${queryVideoList.checked==1 ? 'active':'' }" id="li3">
      <a href="javascript:void(0)" onclick="typeserch(1,'li3');">已通过视频</a>
      </li>
      <li class="filter-item ${queryVideoList.checked==2 ? 'active':'' }" id="li4">
      <a href="javascript:void(0)" onclick="typeserch(2,'li4');">未通过视频</a>
      </li>
					...
					<div class="tags">
				     ...
				     <c:if test="${clist.uuid==list.uuid}">${clist.count}</c:if>
						...
						<span class="so-icon" title="up主">
							...
							<a href="//space.bilibili.com/11082854?from=search&amp;seid=9141651883842617988" class="up-name" target="_blank" se-linkid="video_1_2634507_up">${list.uploadUser}</a>
						</span>
					</div>
					 <c:choose>
					             <c:when test="${list.checked==0}"><img alt="##" src="source/videiunchecked.png" class="checkimg" id="labelforcheck${ite.count}"></c:when>
					             <c:when test="${list.checked==1}"><img alt="##" src="source/videopass.png" class="checkimg" id="labelforcheck${ite.count}"></c:when>
					             <c:when test="${list.checked==2}"><img alt="##" src="source/nopass.png" class="checkimg" id="labelforcheck${ite.count}"></c:when>
					</c:choose>
      ...
      <ul class="dropdown-menu" style=" min-width: 65px; height: auto;">
      <c:if test="${list.checked==0||list.checked==2}"><li><a href="#myModal${ite.count}"  data-toggle="modal"  >修改</a></li></c:if>
    
       <li><a href="#foo${ite.count}" data-toggle="modal" >删除</a></li>
     </ul>
    </div>
				</div>
